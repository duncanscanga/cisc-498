...
def get_course_details(user, courseId):
    ...
    students = []
    if user.role == 2:
        students = findAssignedStudents(user, user.id, courseId)

    return render_template('course-details.html', users=users, students=students, user=user, course=course, assignments=assignments)
